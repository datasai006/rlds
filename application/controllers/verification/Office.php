...
public function update_office($id) {
    ...
    if ($this->input->post()) {
       ...
       'branch_name' => $this->input->post('branch_name'),
        ...
        // Update signature and stamp if new files were uploaded
        if ($signature_data) {
            $data['signature'] = $signature_data['file_name'];
            // Delete old signature file if exists
            if (!empty($existing_data->signature)) {
                unlink($config['upload_path'] . $existing_data->signature);
            }
        }
        ...
        if ($stamp_data) {
            ...
            if (!empty($existing_data->stamp)) {
                unlink($config['upload_path'] . $existing_data->stamp);
            }
        }
