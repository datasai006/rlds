           ...
           <div class="p-3">
               <table class="table table-bordered">
                   ...
                   <tbody>
                       ...
                       <tr>
                           <td><?= is_array($employee) && array_key_exists('id', $employee) ? $employee['id'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('last_name', $employee) ? $employee['last_name'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('email', $employee) ? $employee['email'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('gendar', $employee) ? $employee['gendar'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('role_id', $employee) ? $employee['role_id'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('doj', $employee) ? $employee['doj'] : 'N/A' ?>
                           </td>
                           <td><?= is_array($employee) && array_key_exists('mobile_no', $employee) ? $employee['mobile_no'] : 'N/A' ?>
                           </td>
                           <td style="color: blue">
                               <!-- <a href="#"><i class="bi bi-printer"></i></a> | -->
                               <a
                                   href="<?= base_url() ?>Employee/Employe/edit_employee/<?= is_array($employee) && array_key_exists('id', $employee) ? $employee['id'] : '' ?>"><i
                                       class=" bi bi-pencil-square"></i></a> |
                               <a
                                   href="<?= base_url(); ?>Employee/Employe/delete_employee/<?= is_array($employee) && array_key_exists('id', $employee) ? $employee['id'] : '' ?>">
